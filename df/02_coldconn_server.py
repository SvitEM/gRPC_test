#!/usr/bin/env python3
"""
DF Cold Connection Server - gRPC server with mTLS and artificial delay
Identical implementation to steady server but with different naming for paired execution
"""
import asyncio
import logging
import os
...
async def serve():
    ...
    logging.info(f"Starting DF Cold Connection Server on {host}:{port} with {delay_ms}ms delay")
    ...
    # Create and configure server with SO_REUSEPORT
    server = grpc.aio.server(
        options=(
            ('grpc.so_reuseport', 1),            # Enable SO_REUSEPORT for horizontal scaling
    ...
    # Start server
    await server.start()
    logging.info(f"DF Cold Connection Server listening on {listen_addr} (mTLS enabled)")
